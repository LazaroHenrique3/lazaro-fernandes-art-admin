...
const getAll = async (page = 1, filter = '', status = '', showInative = false, id?: number): Promise<ICategoryTotalCount | Error> => {
    let relativeUrl = ''

    if (id) {
        relativeUrl = `/category?id=${id}&page=${page}&limit=${Environment.LINE_LIMIT}&showInative=${showInative}&filter=${filter}&status=${status}`
    } else {
        relativeUrl = `/category?page=${page}&limit=${Environment.LINE_LIMIT}&showInative=${showInative}&filter=${filter}&status=${status}`
    }
...
const generatePdf = async (filter = '', status = ''): Promise<Uint8Array | Error> => {
    ...
    try {
        const relativeUrl = `/category/report/generate?filter=${filter}&status=${status}`
